...
namespace DependencyInjection.XmlExtensions
{
    ...
    public class XmlInterpreter : IFileInterpreter
    {
        ...
        protected static readonly string LifestyleAttributeName = "lifestyle";
        ...
        private List<Component> MyFunction(XmlNode rootNode)
        {
            ...
            foreach (XmlNode node in componentsNode.ChildNodes)
            {
                ...
                var type = node.Attributes[TypeAttributeName];
                var service = node.Attributes[ServiceAttributeName];
                var lifestyle = node.Attributes[LifestyleAttributeName];
